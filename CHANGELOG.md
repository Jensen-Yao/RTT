...
- Improve Web shared-audio capture retries and stop cleanup so a recorder failure does not stall a session.
- Split the Android live-caption workspace into Session and Configure panels for clearer setup and operation.
- Keep Windows usable when local runtime or startup initialization fails, with a visible diagnostic and persistent startup log.
