...
RTT 为 Android 10+ 与 Windows 10 2004+ x64 提供实时原文和简体中文字幕。Android 可选择语音、画面字幕 OCR 或二者同时启用；SenseVoice 支持英语、日语、韩语，云端固定语言还可选择更多语言。专业翻译机还能从视频音轨生成带时间轴的翻译字幕。RTT 不是视频播放器，不修改原视频，也不提供口型同步。
...
在三端的“翻译机 → 视频”选择本地视频后，RTT 会提取音轨、统一为 16 kHz 单声道 PCM、检测语音段、调用当前 ASR，并按时间轴逐段翻译。可输出 `SRT`、`VTT`、`ASS` 或 `TXT`，并选择只保留译文或同时保留原文和译文。任务页会显示解码、转写、翻译和生成文件的进度，完成后可预览并下载。

本地 ASR 时视频、音轨、转写和译文都留在设备上。选择云端 ASR 时必须先勾选上传同意，RTT 只发送分段后的音轨，绝不发送视频帧；后续文字翻译 API 会收到识别出的字幕文本。Android 长视频使用自动清理的临时 PCM 文件，页面切换不会取消当前视频任务。Windows 使用系统 Media Foundation，MP4、MOV、M4V 和 AVI 通常可直接解码，MKV/WebM 是否可用取决于系统编解码器。
...
- 视频转字幕使用云端 ASR 时只上传已确认的分段音轨，视频画面始终不上传。
